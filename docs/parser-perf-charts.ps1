...
$cPrior = '#9AA3AF'; $cRecent = '#0072B2'; $cStable = '#009E73'
...
for ($i = 0; $i -lt $N; $i++) {
  ...
  $x = 73 + $slot * $i
  $h = $b.v * $px; $by = $y0 - $h
  $col = if ($b.g -eq 'prior') { $cPrior } elseif ($b.g -eq 'recent') { $cRecent } else { $cStable }
  $cx = n($x + $barW / 2)
...
# legend (3 groups)
$l1 = $chartRight - 520; $l2 = $chartRight - 330; $l3 = $chartRight - 150
...
# =========================================================================================
$buckets = @(
  @{ name='All';    start=31.22; end=18.70; col='#0072B2' }
  @{ name='Raw';    start=19.25; end=11.47; col='#D55E00' }
  @{ name='Select'; start=11.09; end=5.05;  col='#009E73' }
  @{ name='Table';  start=3.20;  end=2.29;  col='#E69F00' }
)
...
$p2 += "<text x='64' y='30' font-size='17' font-weight='700' fill='#111827'>Recent wins (11-19) &#8212; allocation remaining per corpus bucket (lower is better)</text>"
$p2 += "<text x='64' y='50' font-size='12' fill='#6b7280'>before (start, grey) vs after the recent wins (coloured), normalized to each bucket's start = 100%</text>"
...
$p2 += "<rect x='996' y='66' width='14' height='14' fill='#0072B2'/><text x='1016' y='78' font-size='12' fill='#374151'>after the recent wins</text>"
$p2 += "</svg>"
