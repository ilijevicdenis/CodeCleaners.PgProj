...
- ⬜ **#151** (child of #134 EP-EXTRACT) — `.pgpkg`-embedded `data/` **COPY section** (BACPAC-analogue variant) — the last open piece of data extract. **Note:** COPY FROM cannot accept explicit values for `GENERATED ALWAYS AS IDENTITY` columns (the INSERT seed path uses `OVERRIDING SYSTEM VALUE`); the loader must temporarily flip such columns to `BY DEFAULT` (or fall back to INSERT) — needs a live-DB round-trip to validate.
