...
/// stays green on machines without Docker/Postgres.
///
/// Each run gets its OWN throwaway database (via <see cref="ThrowawayDatabaseFixture"/>), eliminating the
/// shared-state extension collision (btree_gist dup-key) described in issue #37.
/// </summary>
public sealed class JsonContractIntegrationTests : IClassFixture<ThrowawayDatabaseFixture>
{
    private readonly ThrowawayDatabaseFixture _fixture;

    public JsonContractIntegrationTests(ThrowawayDatabaseFixture fixture)
        => _fixture = fixture;
    ...
    public async Task Compare_json_matches_text_plan_against_live_db()
    {
        ...
        // No manual extension cleanup needed: each test class has its own throwaway DB.
